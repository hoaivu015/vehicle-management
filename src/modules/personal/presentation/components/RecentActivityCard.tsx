    ...
    >
      ...
      <div className="p-4 md:p-8 space-y-4 md:space-y-5 max-h-[600px] overflow-y-auto custom-scrollbar">
        {activities.slice(0, 10).map((activity, i) => (
          <div key={i} className="flex items-center justify-between p-3.5 md:p-5 rounded-t2 bg-white/40 border border-white/60 hover:border-kraft-accent/20 transition-all duration-300 group shadow-sm hover:shadow-md cursor-pointer">
            <div className="flex items-center gap-3 md:gap-5 min-w-0">
              <div className={cn(
                "w-10 h-10 md:w-12 md:h-12 rounded-t3 shadow-sm flex items-center justify-center shrink-0 border transition-all duration-300 group-hover:scale-110",
              ...
              <div className="min-w-0">
                <p className="font-black text-xs md:text-sm text-kraft-ink tracking-tight group-hover:text-kraft-accent transition-colors truncate">{activity.carName}</p>
                <p className="text-[10px] font-black uppercase tracking-widest mt-0.5 md:mt-1 opacity-40 flex items-center gap-1.5 md:gap-2 whitespace-nowrap">
            ...
            <div className="flex items-center gap-3 md:gap-4 shrink-0 pl-2">
              <div className="text-right">
                <p className="font-black text-sm md:text-base text-kraft-ink tracking-tight whitespace-nowrap">
                  +{formatCurrency(activity.amount)}
                </p>
                <p className="text-sub-label !opacity-30 whitespace-nowrap">Hoa hồng</p>
              </div>
