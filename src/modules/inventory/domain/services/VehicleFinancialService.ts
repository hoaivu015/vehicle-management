...
import { calcProfitShare, calcRefundablePartnerCapital } from '@/src/shared/utils/financial_formulas';

/**
 * Vehicle Calculation Domain Service & Utilities
 * Project: Auto-28
 * Goal: Centralized Single Source of Truth (SSoT) for vehicle metrics, inventory aging, and financials.
 */
...
export interface VehicleFinancials {
  ...
  refundablePartnerCapital: number; // Vốn thực nhận sau khi bán (đã trừ lỗ nếu bán cắt lỗ)
...
export const calculateVehicleFinancials = (vehicle: FinancialInput): VehicleFinancials => {
  ...
  const refundablePartnerCapital = isCoinvested 
    ? calcRefundablePartnerCapital(coinvestAmount, partnerProfitShare, isSold)
    : 0;
...
/**
 * Tính công nợ phải thu từ khách hàng (Receivable Debt) theo chuẩn SSoT.
 * Áp dụng cho các giai đoạn bán xe: DEPOSIT_SALE, BANK_DEPOSIT, BANK_CONFIRMED, SOLD
 */
export const calcVehicleReceivableDebt = (vehicle: {
  status: VehicleStatus;
  sale_price?: number | null;
  received_amount?: number | null;
}): number => {
  ...
  if (!isSalePhase) return 0;
  const salePrice = vehicle.sale_price || 0;
  const receivedAmount = vehicle.received_amount || 0;
  return Math.max(0, salePrice - receivedAmount);
};

/**
 * Tính công nợ phải trả cho chủ cũ / đơn vị cung cấp xe (Payable Debt) theo chuẩn SSoT.
 */
export const calcVehiclePayableDebt = (vehicle: {
  purchase_price?: number | null;
  purchase_paid_amount?: number | null;
}): number => {
  const purchasePrice = vehicle.purchase_price || 0;
  const paidAmount = vehicle.purchase_paid_amount || 0;
  return Math.max(0, purchasePrice - paidAmount);
};
